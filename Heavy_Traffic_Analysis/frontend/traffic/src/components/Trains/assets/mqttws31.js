...
Messaging = (function (global) {
  ...
  ClientImpl.prototype._on_socket_message = function (event) {
    ...
    try {
      ...
      this._disconnected(
        ...
        format(ERROR.INTERNAL_ERROR, [error.message])
      );
      return;
    }
  ...
  ClientImpl.prototype._disconnected = function (errorCode, errorText) {
    ...
    if (
      this.connectOptions.hosts &&
      this.hostIndex < this.connectOptions.hosts.length - 1
    ) {
      // Try the next host.
      this.hostIndex++;
      this._doConnect(
        this.connectOptions.hosts[this.hostIndex],
        this.connectOptions.ports[this.hostIndex]
      );
    } else {
  ...
  var Client = function (host, port, clientId) {
    ...
    if (
      ...
      (clientIdLength < 1) | (clientIdLength > 23)
    )
    ...
    this.send = function (message) {
      ...
      if (typeof message.destinationName === "undefined")
        throw new Error(
          "Invalid parameter Message.destinationName:" + message.destinationName
        );
